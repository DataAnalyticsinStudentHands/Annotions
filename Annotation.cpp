...
void Annotation::saveAnnot()
{
    ...
    if (fp1)
    {
        fprintf(fp1, "Owner of annotation: ", owner);
        fprintf(fp1, "# x,      y,      z,      dx,      dy,     dz\n");
        fprintf(fp1, "Annotation: ", text); //text variable from ln 271. Would need to implement function on line 275 to make this work
        
        start = std::chrono::system_clock::now(); //USE CHRONO LIBRARY TO ADD DATE AND TIME.
    ...
    //merge two files into a third file. annotation and record path files
    //CHECK FOR ALTERNATE INSERTION TO FILE
    ...
    fprintf(fp3, "FILES WERE MERGED CORRECTLY FROM ANNOTATION AND RECORD PATH PLUGIN");
...
// create new function to merge two files
